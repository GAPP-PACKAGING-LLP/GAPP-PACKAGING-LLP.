      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="bg-brand-primary-hover text-white rounded-2xl p-8 sm:p-12 border border-teal-800 space-y-4 shadow-lg">
          ...
          <div className="pt-4 flex flex-wrap items-center gap-3">
            <button
              ...
              className="bg-brand-accent hover:bg-brand-accent-hover text-white px-6 py-3 rounded-md font-bold text-xs uppercase tracking-wider transition-colors shadow cursor-pointer"
            >
              ...
              >
                <div className="shrink-0">
                  {director.photoUrl ? (
                    <img
                      ...
                      loading="lazy"
                      decoding="async"
                      className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover border-2 border-brand-primary/20 shadow-sm"
                    />
                  ) : (
                    <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl bg-linear-to-br from-brand-primary to-[#0A2540] text-white flex items-center justify-center font-black text-3xl shadow-sm">
                ...
                <div className="space-y-3 flex-1">
                  <div>
                    ...
                    <p className="text-xs font-bold text-brand-primary">{director.role}</p>
                    {director.din && (
                  ...
                  <div className="pt-2 border-t border-slate-100 space-y-1.5 text-xs text-slate-700">
                    {director.phone && (
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`tel:${director.phone}`} className="font-mono hover:text-brand-primary font-semibold">
                          {director.phone}
                        </a>
                      </div>
                    )}
                    {director.email && (
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`mailto:${director.email}`} className="hover:text-brand-primary truncate">
                          {director.email}
                        </a>
                      </div>
                    )}
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 sm:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Link
              to="/clients"
              className="text-xs font-bold text-brand-primary hover:text-brand-accent flex items-center gap-1 shrink-0"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4">
            {activeClients.slice(0, 10).map((client, idx) => (
              <div
                key={client.id || idx}
                className="bg-white rounded-xl p-3 border border-slate-200 flex flex-col items-center justify-between text-center hover:border-brand-primary/40 hover:shadow-xs transition-all min-h-[105px]"
              >
