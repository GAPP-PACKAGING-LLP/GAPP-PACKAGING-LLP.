  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6" id="brochure-modal-overlay">
      <div className="relative bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[92vh] overflow-y-auto border border-slate-200 print:max-w-none print:shadow-none print:border-none">
        ...
        {/* Top Sticky Bar */}
        <div className="sticky top-0 z-20 bg-brand-primary text-white px-6 py-4 flex items-center justify-between border-b border-teal-800 print:hidden">
        ...
        <div className="p-6 sm:p-8 space-y-8 bg-white text-slate-800">
          ...
          <div className="space-y-3">
            <h2 className="text-sm font-bold text-brand-primary flex items-center gap-2 border-b border-slate-200 pb-1.5">
              <Users className="w-4 h-4 text-brand-accent" />
              <span>Designated Partners (Page 9)</span>
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {designatedPartnersData.map((p, i) => (
                <div key={i} className="p-3 bg-brand-primary/5 border border-brand-primary/15 rounded-md text-xs">
                  <div className="font-bold text-slate-900">{p.name}</div>
                  <div className="text-slate-500">{p.role}</div>
                  <div className="font-mono font-semibold text-brand-primary mt-1">{p.phone}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {machineryData.map((m, idx) => (
                <div key={m.id} className="p-2.5 bg-slate-50 border border-slate-200 rounded flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-brand-primary font-bold text-[11px]">{idx + 1}.</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              {clientPartnersData.map((c) => (
                <div key={c.id} className="p-2 bg-slate-50 border border-slate-200 rounded flex items-center justify-between">
                  ...
                  {c.details && <span className="text-[10px] text-brand-accent font-semibold">{c.details}</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Addresses Footer */}
          <div className="bg-brand-primary text-white p-4 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-xs">
            <div>
