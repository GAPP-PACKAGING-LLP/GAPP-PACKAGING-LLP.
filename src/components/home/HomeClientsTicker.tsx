      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {displayClients.map((client, idx) => (
            <div
              key={client.id || idx}
              className="bg-[#F8F9FA] rounded-xl p-4 sm:p-5 border border-slate-200 text-center flex flex-col justify-between items-center hover:bg-white hover:border-brand-primary/50 hover:shadow-md transition-all duration-300 min-h-[140px] group"
            >
              {/* Logo Area */}
              <div className="w-full h-14 flex items-center justify-center mb-2 px-2">
                {client.logoUrl ? (
                  ...
                  <div className="w-10 h-10 rounded-lg bg-teal-50 text-brand-primary flex items-center justify-center font-bold text-xs">
              ...
              <div className="w-full space-y-1">
                <div className="text-xs sm:text-xs font-bold text-slate-800 leading-tight group-hover:text-brand-primary transition-colors line-clamp-2">
                  {client.name}
                </div>
                {client.details && (
                  <div className="text-[10px] font-semibold text-brand-primary truncate">
                    {client.details}
                  </div>
                )}
                {client.sector && (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
          ...
          <Link
            to="/clients"
            className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-brand-primary hover:text-brand-accent group"
          >
